...
class MLPDiffusion(nn.Module):
    def __init__(self, n_steps, num_units=128):
        ...
        self.net = nn.Sequential(
            ...
            nn.Linear(num_units, num_units),
            nn.ReLU(),
            nn.Linear(num_units, 3),  # Output dimensions (x1, x2, x3)
        )
...
model = MLPDiffusion(num_steps, num_units=128).to(device)
...
# Generate samples
generated_samples_seq = p_sample_loop(model, (1000, 3), num_steps, betas, one_minus_alphas_bar_sqrt, device)
